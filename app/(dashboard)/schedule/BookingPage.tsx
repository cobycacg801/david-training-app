...
export default function BookingPage({
  ...
  // Availability state
  const [closedDays,  setClosedDays]  = useState<Set<number>>(new Set([0])); // Sun closed by default
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Parse slot string e.g. "9:00 AM" → build ISO datetime
    const [timePart, ampm] = selectedSlot.split(" ");
    let [h, m] = timePart.split(":").map(Number);
    if (ampm === "PM" && h !== 12) h += 12;
    if (ampm === "AM" && h === 12) h = 0;
    const dt = new Date(`${selectedDate}T${String(h).padStart(2,"0")}:${String(m).padStart(2,"0")}:00`);
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 28 }}>
      ...
      <div>
        ...
        <p style={{ fontSize: 13, color: "#a1a1aa" }}>
          Choose an available day, pick your time slot, and book directly with Coach David.
        </p>
      </div>
              ...
              <div key={b.id} style={{
                ...
                <div style={{ display: "flex", gap: 14, alignItems: "flex-start" }}>
                  ...
                  <div>
                    ...
                    <p style={{ fontSize: 12, color: "#71717a", marginBottom: 6 }}>{fmt(b.scheduled_at)} · {fmtTime(b.scheduled_at)}</p>
        ...
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 22 }}>
          ...
          {selectedDate && (
            <div>
              <label style={{ fontSize: 10, fontWeight: 700, color: "#52525b", letterSpacing: 1, textTransform: "uppercase", display: "block", marginBottom: 10 }}>
                ...
                {selectedDate && <span style={{ color: "#3f3f46", fontWeight: 400, textTransform: "none", marginLeft: 8 }}>
                  ...
                  {slots.map(slot => (
                    <button
                      ...
                      style={{
                        ...
                        background: selectedSlot === slot ? "rgba(201,168,76,0.15)" : "rgba(255,255,255,0.03)",
                        border: `0.5px solid ${selectedSlot === slot ? "rgba(201,168,76,0.4)" : "rgba(255,255,255,0.1)"}`,
                        fontSize: 13, fontWeight: 600,
                        color: selectedSlot === slot ? G : "#a1a1aa",
                        transition: "all 0.15s",
                      }}
                    >
                      {slot}
          ...
          <button
            ...
            style={{
              ...
              background: submitting || !selectedDate || !selectedSlot
                ? "rgba(255,255,255,0.03)"
                : `linear-gradient(135deg, rgba(201,168,76,0.15), rgba(201,168,76,0.28))`,
              border: `0.5px solid ${submitting || !selectedDate || !selectedSlot ? "rgba(255,255,255,0.07)" : "rgba(201,168,76,0.4)"}`,
              fontSize: 14, fontWeight: 800,
              color: submitting || !selectedDate || !selectedSlot ? "#3f3f46" : G,
              cursor: submitting || !selectedDate || !selectedSlot ? "not-allowed" : "pointer",
              transition: "all 0.2s",
            } as React.CSSProperties}
          >
              ...
              return (
                <div key={b.id} style={{ background: "rgba(255,255,255,0.01)", border: "0.5px solid rgba(255,255,255,0.05)", borderRadius: 12, padding: "14px 18px", display: "flex", alignItems: "center", justifyContent: "space-between", opacity: 0.6 }}>
                  <p style={{ fontSize: 13, fontWeight: 600, color: "#a1a1aa" }}>
                    {b.session_type === "online" ? "Online" : "In-Person"} · {fmt(b.scheduled_at)} {fmtTime(b.scheduled_at)}
                  </p>
